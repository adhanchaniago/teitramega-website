...
class Template
{
    ...
    public function view($content, $data = NULL)
    {
        if ( ! $content)
        {
            ...
            $this->template['header']          = $this->ci->load->view('public/template/header', $data, TRUE);
            $this->template['navbar']     = $this->ci->load->view('public/template/navbar', $data, TRUE);
            $this->template['left_sidebar']     = $this->ci->load->view('public/template/left_sidebar', $data, TRUE);
            $this->template['content']         = $this->ci->load->view($content, $data, TRUE);
            $this->template['right_sidebar']     = $this->ci->load->view('public/template/right_sidebar', $data, TRUE);
            $this->template['footer']          = $this->ci->load->view('public/template/footer', $data, TRUE);

            return $this->ci->load->view('public/template/template', $this->template);
        }
    }
